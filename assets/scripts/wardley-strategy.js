...
function wardleyStrategyGenerator(){
    ...
    var result = template.replace(/<%=adjective%>/g, function() { return adjectives.splice(Math.floor(Math.random() * adjectives.length), 1); });
 result = result.replace(/<%=indef_adjective%>/g, function() { return generateIndefiniteArticlePrefix(adjectives.splice(Math.floor(Math.random() * adjectives.length)), 1); });
    result = result.replace(/<%=noun%>/g, function() { return nouns.splice(Math.floor(Math.random() * nouns.length), 1); });
    result = result.replace(/<%=plural_noun%>/g, function() { return pluralNouns.splice(Math.floor(Math.random() * pluralNouns.length), 1); });
    result = result.replace(/<%=indef_noun%>/g, function() { return nouns.splice(Math.floor(Math.random() * nouns.length), 1); });
...
function generateIndefiniteArticlePrefix(inputWord){
    ...
    if ( testString.toLowerCase().at(0) === 'a'
    || testString.toLowerCase().at(0) === 'e'
    || testString.toLowerCase().at(0) === 'i'
    || testString.toLowerCase().at(0) === 'o'
    || testString.toLowerCase().at(0) === 'u' ) {
